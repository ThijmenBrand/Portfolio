...
function make(p: p5) {
    ...
    const drawLine = (cords1: { x: number, y: number }, cords2: { x: number, y: number }, weight = 0.1) => {
        p.stroke("white");
    ...
    p.draw = function () {
        ...
        for (let i = 0; i < dots.length; i++) {
            ...
            if (this.dist(dotI.posX, dotI.posY, mouse.x, mouse.y) < 100) {
                drawLine({ x: dotI.posX, y: dotI.posY }, { x: mouse.x, y: mouse.y }, 0.5)
            }

            for (let j = 0; j < dots.length; j++) {
                var dotII = dots[j];
                if (this.dist(dotI.posX, dotI.posY, dotII.posX, dotII.posY) < 100) {
                    drawLine({ x: dotI.posX, y: dotI.posY }, { x: dotII.posX, y: dotII.posY });
                }
            }
        }
    }
...
onMounted(() => {
    ...
    meshThingElement.addEventListener('mousemove', function (e) {
        mouse.x = e.clientX;
        mouse.y = e.clientY - meshThingElement.offsetTop;
    });
